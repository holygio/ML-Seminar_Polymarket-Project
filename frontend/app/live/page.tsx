...
import { fetchLive, fmtPrice, fmtProb, fmtRet, polyColor, retColor, type LiveRecord } from '@/lib'
      ...
      <div style={{
        ...
        {duped.map((rec, i) => (
          <div key={i} style={{
            ...
            <div style={{ fontSize: '8px', letterSpacing: '1.5px', color: '#1e3a5f' }}>
              US EQUITIES
            </div>
          </div>
        ))}
...
function EquityTile({
  ...
  const stockOverlay =
    record.ret_pct !== null && record.ret_pct > 0.1
      ? 'linear-gradient(135deg, rgba(34,197,94,0.32) 50%, transparent 50%)'
      : record.ret_pct !== null && record.ret_pct < -0.1
        ? 'linear-gradient(135deg, rgba(239,68,68,0.34) 50%, transparent 50%)'
        : 'linear-gradient(135deg, rgba(100,116,139,0.14) 50%, transparent 50%)'
  const polyOverlay =
    record.poly_up_probability !== null && record.poly_up_probability > 0.55
      ? 'linear-gradient(135deg, transparent 50%, rgba(34,197,94,0.28) 50%)'
      : record.poly_up_probability !== null && record.poly_up_probability < 0.45
        ? 'linear-gradient(135deg, transparent 50%, rgba(239,68,68,0.3) 50%)'
        : 'linear-gradient(135deg, transparent 50%, rgba(100,116,139,0.12) 50%)'
          ...
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '10px', color: polyColor(record.poly_up_probability) }}>
                  {fmtProb(record.poly_up_probability)}
                </div>
                <div style={{ fontSize: '7px', letterSpacing: '1px', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginTop: '1px' }}>
                  Poly up
                </div>
              </div>
            </div>
  ...
  return (
    <div style={{ marginBottom: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
        ...
        {timingText && (
          <span style={{ fontSize: '8px', color: '#1e3a5f', letterSpacing: '0.5px' }}>
            {timingText}
          </span>
        )}
      </div>
...
export default function LivePage() {
  ...
  if (loading && data.length === 0) {
    return (
      <div style={{
        background: '#080e1a', minHeight: '100vh', display: 'flex', 
        alignItems: 'center', justifyContent: 'center',
        fontFamily: "'Courier New', monospace", fontSize: '14px', color: '#334155'
      }}>
        Warming up
        <style>{`
  ...
  return (
    <div style={{ background: '#080e1a', minHeight: '100vh', fontFamily: "'Courier New', monospace" }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <span style={{ fontSize: '9px', color: '#334155', letterSpacing: '1.5px', textTransform: 'uppercase' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '10px', color: '#475569' }}>
          {loading ? (
            <div style={{ width: '8px', height: '8px', border: '1px solid #334155', borderRadius: '50%', borderTopColor: '#f1f5f9', animation: 'spin 1s linear infinite' }} />
          ) : (
      ...
      <div style={{
        ...
        <div style={{ background: '#080e1a', padding: '10px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
            ...
            <span style={{ fontSize: '8px', color: '#334155', marginLeft: 'auto' }}>sized by market cap</span>
          </div>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              {[
                ...
                <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  ...
                  <span style={{ fontSize: '8px', color: '#334155' }}>{item.label}</span>
                </div>
              ))}
            </div>
          </div>
