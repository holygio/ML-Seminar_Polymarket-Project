...
def build_probability_series(
    ...
    """
    Build the probability_series for the asset chart.

    For 1D: return all 5-minute intraday points for the most recent
            trading day present in the data.

    For 7D / 30D / 60D: return one point per calendar day — the last
            Polymarket probability reading before 09:30 AM New York time
            (the pre-open research signal). Days with no pre-open data
            are skipped silently.

    Args:
        df:   DataFrame filtered to a single ticker, already filtered
              to the relevant date range. Must contain TIMESTAMP (UTC),
              avg_price_up (or close_bet), total_volume, open_bet,
              high_bet, low_bet.
        days: the requested time range (1, 7, 30, or 60).

    Returns:
        List of dicts with timestamp, price_up, volume, open_bet,
        high_bet, low_bet.
    """
    if df.empty:
        return []
    ...
    # ── Convert to New York time for pre-open filtering ───────────────
    ...
    # ── 1D: full intraday for the most recent day ─────────────────────
    if days == 1:
        most_recent_date = df["date_ny"].max()
        intraday = df[df["date_ny"] == most_recent_date].copy()
    ...
    # ── Multi-day: one pre-open point per day ─────────────────────────
    # Pre-open = strictly before 09:30 AM New York time
    pre_open_mask = (
        (df["hour_ny"] < 9)
        | ((df["hour_ny"] == 9) & (df["minute_ny"] < 30))
    )
    pre_open_df = df[pre_open_mask].copy()

    if pre_open_df.empty:
        # Fallback: no pre-open data available (e.g. very old data)
        # Use the first reading of each day as the best approximation
        daily_df = (
            df.sort_values("ts_ny")
            .groupby("date_ny", sort=True)
            .first()
            .reset_index()
        )
    else:
        # Last pre-open reading per day = the research signal
        # (closest to 09:30 ET, most information before open)
        daily_df = (
            pre_open_df.sort_values("ts_ny")
            .groupby("date_ny", sort=True)
            .last()
            .reset_index()
        )

    daily_df = daily_df.sort_values("date_ny")

    return [
        {
            # Use date string for multi-day (cleaner x-axis labels)
            "timestamp": row["date_ny"].isoformat(),
            "price_up":  nan_safe(row.get("close_bet")),
            "volume":    nan_safe(row.get("total_volume")),
            "open_bet":  nan_safe(row.get("open_bet")),
            "high_bet":  nan_safe(row.get("high_bet")),
...
def _derive_equity_exposure(title: str) -> dict[str, str]:
    """
    Derive per-ticker exposure level from event title.
    Uses tiered keyword matching: HIGH for direct/strong exposure,
    MED for indirect/sector exposure.
    Returns {ticker: "HIGH"|"MED"} for affected equities only.
    """
    t = (title or "").lower()

    TICKER_RULES = [
        ("NVDA", [
            "nvidia", "chip", "semiconductor", "gpu", "ai chip",
            "export control", "chip ban", "chip restriction",
        ], [
            "china tech", "tech tariff", "ai regulation",
            "rate cut", "recession", "tariff",
        ]),
        ("AAPL", [
            "apple", "iphone", "china supply", "tariff on china",
            "us china trade", "foxconn",
        ], [
            "trade deal", "china tariff", "tech tariff",
            "rate cut", "recession",
        ]),
        ("MSFT", [
            "microsoft", "azure", "ai regulation", "eu ai",
            "ukraine", "russia sanction", "ceasefire",
        ], [
            "rate cut", "recession", "trade deal",
            "china", "antitrust",
        ]),
        ("AMZN", [
            "amazon", "aws", "tariff on goods",
            "trade war", "trade deal", "consumer tariff",
        ], [
            "rate cut", "recession", "antitrust",
        ]),
        ("GOOGL", [
            "google", "alphabet", "antitrust", "eu regulation",
            "search regulation", "ad regulation",
        ], [
            "rate cut", "recession", "ai regulation",
            "ukraine", "geopolit",
        ]),
        ("META", [
            "meta", "facebook", "social media regulation",
            "eu digital", "content regulation",
        ], [
            "rate cut", "election", "political risk",
        ]),
        ("TSLA", [
            "tesla", "electric vehicle", "ev tariff", "china ev",
            "elon", "doge", "lithium",
        ], [
            "rate cut", "recession", "trade deal",
            "china", "tariff",
        ...
        ("NFLX", [
            "netflix", "streaming regulation", "content tax",
        ], [
            "recession", "rate cut", "consumer spending",
        ]),
        ("PLTR", [
            "palantir", "defense contract", "government ai",
            "war", "military", "ukraine", "ceasefire", "conflict",
        ], [
            "political risk", "election", "sanction",
        ]),
        ("COIN", [
            "bitcoin", "crypto", "coinbase", "stablecoin",
            "crypto regulation", "sec crypto",
        ], [
            "digital asset", "fintech regulation",
        ]),
    ]

    HIGH_IMPACT_CONTEXT = [
        "sanction", "ban", "block", "restrict", "invade",
        "invasion", "war", "ceasefire", "tariff", "export control",
    ]
    has_high_context = any(kw in t for kw in HIGH_IMPACT_CONTEXT)

    exposure: dict[str, str] = {}
    for ticker, high_kws, med_kws in TICKER_RULES:
        if any(kw in t for kw in high_kws):
            exposure[ticker] = "HIGH"
        elif any(kw in t for kw in med_kws):
            exposure[ticker] = "HIGH" if has_high_context else "MED"
...
def _plain_english_interpretation(
    ...
    trend = ""
    if change_24h is not None and abs(change_24h) >= 0.005:
        pp = round(abs(change_24h) * 100, 1)
        direction = "up" if change_24h > 0 else "down"
        trend = f", moved {direction} {pp}pp today"

    if market_type == "categorical" and leading_outcome:
        outcome = leading_outcome.strip('"').strip("'")
        if pct >= 50:
            return (
                f"Crowd expects <strong>{outcome}</strong> as the most "
                f"likely outcome ({pct}%){trend}."
            )
        elif pct >= 30:
            return (
                f"{pct}% chance of <strong>{outcome}</strong> — "
                f"crowd uncertain across multiple outcomes{trend}."
            )
        else:
            return (
                f"Most likely: <strong>{outcome}</strong> at {pct}% — "
                f"no dominant outcome, crowd highly dispersed{trend}."
            )

    if probability >= 0.80:
        return (
            f"Crowd strongly expects this ({pct}%)"
            + (trend or ", high conviction") + "."
        )
    elif probability >= 0.65:
        return f"Crowd leans toward this happening ({pct}%){trend}."
    elif probability >= 0.55:
        return (
            f"Slight majority expects this ({pct}%) — "
            f"outcome still contested{trend}."
        )
    elif probability >= 0.45:
        return (
            f"Outcome is genuinely uncertain ({pct}%) — "
            f"close to coin-flip territory{trend}."
        )
    elif probability >= 0.30:
        return f"Crowd leans against this ({pct}%){trend}."
    elif probability >= 0.15:
        return f"Crowd strongly doubts this ({pct}%){trend}."
    else:
        return (
            f"Crowd assigns low probability ({pct}%) — "
            f"tail risk only{trend}."
        )
...
def asset_detail(ticker: str, days: int = Query(default=1, ge=1, le=60)):
    ...
    # Build probability series: 1D = full intraday, multi-day = one pre-open point per day
    prob_series = build_probability_series(asset_panel, days)
        ...
        records.append(
            {
                ...
                "open_price":          nan_safe(row.get("open_price")),
                "true_sentiment":      nan_safe(row.get("true_sentiment")),
                "bs_neutral_prob":     nan_safe(row.get("bs_neutral_prob")),
                "sigma_live":          nan_safe(row.get("sigma_live")),
